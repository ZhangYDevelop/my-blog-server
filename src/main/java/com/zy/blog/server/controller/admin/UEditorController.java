...
import com.zy.blog.server.util.Upload;
...
public class UEditorController {
    @Value(value="${global.setting.ueditor}")    //后台图片保存地址
                ...
                try {
                    ...
                    for(MultipartFile pic: files.values()){
                        ...
                        if(!"".equals(uploadInfo)){    //如果上传成功
                            ...
                            jo.put("url", "/ueditor/jsp/upload?filePath="+ infoList[2]);//这里的url字段表示的是上传后的图片在图片服务器的完整地址
                        }else{    //如果上传失败
                        }
//
