...
import com.zy.blog.server.enums.ArticleCommentStatus;
import com.zy.blog.server.mapper.ArticleCategoryRefMapper;
import com.zy.blog.server.mapper.ArticleMapper;
import com.zy.blog.server.mapper.ArticleTagRefMapper;
...
//        }
    ...
    public void insertArticle(Article article) {
        ...
        articleMapper.insertData(article);
        //添加分类和文章关联
        for (int i = 0; i < article.getCategoryList().size(); i++) {
            ArticleCategoryRef articleCategoryRef = new ArticleCategoryRef(article.getArticleId(), article.getCategoryList().get(i).getCategoryId());
            articleCategoryRefMapper.insert(articleCategoryRef);
